...
function valifirstname()
{
  ...
  else
  {
    ...
    //this will initiate the cookie function that creates and sets the value of the fname cookie//
    setCookie("fname", x, 1);
    }
    else
    {
...
function setCookie(cname, cvalue, exdays) 
  {
  const cookieday = new Date();
  cookieday.setTime(cookieday.getTime() + (exdays * 24 * 60 * 60 * 1000));
  let expires = "expires="+cookieday.toUTCString();
  document.cookie = cname + "=" + cvalue + ";" + expires + ";path=/";
  }
// this receives the value of the cookie//
...
//this retrieves a cookie if it has a value and displays it//
function checkCookie() 
  {
    ...
    if (fname != "") 
      {
        cookiealert = "Welcome back "+ fname + ".\nPlease select cancel if you are not "+fname+".";
        if (confirm(cookiealert)) 
          {
           const firstNameField = document.getElementById("fname");
            ...
            // B. Populate ALL other fields from Local Storage (including 'userid')
            localStorageToForm();
          }
        else
          {
...
//Creating fetch for state list
async function populateStates() {
  const selectElement = document.getElementById('state');
  checkCookie();
    try 
    {
      const response = await fetch('state_options.txt');
